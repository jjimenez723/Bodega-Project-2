...
def consolidate_fresh_food_simple():
    ...
    # Manual coordinates for verified locations (approximate Newark addresses).
    manual_coordinates = {
    ...
    verified_fresh = verified_fresh[
        [
            ...
            "Longitude",
            "description",
            "Distance from Rutgers 1",
            "Distances from Rutgers 2",
        ]
    ]
    ...
    for _, row in consolidated_df.iterrows():
        feature = {
            ...
            "properties": {
                ...
                "distance_rutgers_1": str(row["Distance from Rutgers 1"])
                if pd.notna(row["Distance from Rutgers 1"])
                else "",
                "distance_rutgers_2": str(row["Distances from Rutgers 2"])
                if pd.notna(row["Distances from Rutgers 2"])
                else "",
            },
        }
