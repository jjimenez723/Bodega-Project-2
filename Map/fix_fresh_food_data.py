...
def is_in_newark(lat, lng):
    """Check if coordinates are within Newark bounds (approximate)."""
    ...
    return (
        newark_bounds["min_lat"] <= lat <= newark_bounds["max_lat"]
        and newark_bounds["min_lng"] <= lng <= newark_bounds["max_lng"]
    )


def fix_fresh_food_data():
    """Fix fresh food data with geocoding and deduplication."""
    ...
    print(
        f"Processing {len(fresh_food_df)} original locations and {len(verified_fresh)} verified locations"
    )
    ...
    for _, row in fresh_food_df.iterrows():
        name = row["name"]
        lat = row["Latitude"]
        lng = row["Longitude"]
    ...
    for _, row in verified_fresh.iterrows():
        name = row["name"]
    ...
    for _, row in df.iterrows():
        feature = {
            ...
            "properties": {
                ...
                "distance_rutgers_1": str(row["distance_rutgers_1"])
                if pd.notna(row["distance_rutgers_1"])
                else "",
                "distance_rutgers_2": str(row["distance_rutgers_2"])
                if pd.notna(row["distance_rutgers_2"])
                else "",
