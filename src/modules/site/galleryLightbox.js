...
export function initGalleryLightbox() {
  // Set up hero animations, reveal effects, and lightbox interactions.
  onDocumentReady(() => {
    const heroTitle = document.querySelector(".hero-text h1");
    const heroSubtitle = document.querySelector(".hero-text p");

    // Fade in hero copy on load.
    if (heroTitle && heroSubtitle) {
      ...
      window.setTimeout(() => {
        heroTitle.style.transition =
          "opacity 1s cubic-bezier(0.4,0.2,0.2,1), transform 1s cubic-bezier(0.4,0.2,0.2,1)";
    ...
    // Add reveal effects if ScrollReveal is available.
    ...
    // Exit if the lightbox structure is missing.
    if (
    ...
    // Open the lightbox when thumbnails are clicked.
    ...
    // Enable keyboard navigation when open.
